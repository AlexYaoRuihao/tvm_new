...
class SGXModuleNode : public runtime::ModuleNode {
 public:
  ~SGXModuleNode() {
    if (eid_) {
...
.set_body([](TVMArgs args, TVMRetValue* rv) {
  ...
  // ObjectPtr<Object> node = std::make_shared<SGXModuleNode>();
  node->Init(args[0]);
  *rv = runtime::Module(ObjectPtr<Object>(nullptr));
});
